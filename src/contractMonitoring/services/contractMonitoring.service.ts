...
export class ContractMonitoringService implements OnApplicationBootstrap {
    ...
    this.monitoringParameters = [
      {
        ...
        startBlockNumber: BigInt(
          config.contracts.subscriptionManager.deployBlockNumber,
        ),
      ...
      },
      {
        ...
        startBlockNumber: BigInt(
          config.contracts.userManager.deployBlockNumber,
        ),
  ...
  async onApplicationBootstrap(): Promise<void> {
    for (const monitoringParameter of this.monitoringParameters) {
      ...
      if (
        contractInfo.lastProcessedBlock + 1n >
        monitoringParameter.startBlockNumber
      ) {
        monitoringParameter.startBlockNumber =
          contractInfo.lastProcessedBlock + 1n;
      }
    ...
    while (true) {
      ...
      for (const monitoringParameter of this.monitoringParameters) {
        for (
          ...
          const endBlock = min(
            startBlock + BigInt(config.eventPeriod),
